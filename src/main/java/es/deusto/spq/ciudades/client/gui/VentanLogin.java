...
public class VentanLogin extends JFrame {
	...
	private VentanLogin ventanLogin;
	...
	/**
	 * Clase constructora
	 * @param args[] Comando con argumentos. 
	 */
	public VentanLogin(String args[]) {
		try {
			controller = new CiudadesController(args);
		...
		//resourceBundle = ResourceBundle.getBundle("lang/messages", Locale.forLanguageTag(language));
		iniciarComponentes();
	}
	 ...
	 * @param controller Controller de la aplicacion.
 	 * @param resourceBundle Strings de un determinado lenguaje. 
		...
		btnEntrar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(textFieldEmail.getText().trim().equals("") || textFieldPassword.getText().trim().equals("")) {
					JOptionPane.showMessageDialog(null, resourceBundle.getString("emptyFields"),"ERROR! Existen campos vacios", JOptionPane.ERROR_MESSAGE);
					limpiarCampos();
				}
				//Comprobamos si el usuario esta en la bd
				else if(controller.identifyUsuario(textFieldEmail.getText().trim(), textFieldPassword.getText().trim())){
					JOptionPane.showMessageDialog(VentanLogin.this,
							resourceBundle.getString("greetings") + " " + textFieldEmail.getText().trim() + "! " + resourceBundle.getString("welcomeMessage") + " ",
					...
					//userLogeado= controller.identifyUsuario(email, password);
					VentanaPerfilUsuario vpUsuario = new VentanaPerfilUsuario(controller, resourceBundle, userLogeado);
					vpUsuario.centreWindow();
					vpUsuario.setVisible(true);
					dispose();
				}else {
					JOptionPane.showMessageDialog(VentanLogin.this,
							resourceBundle.getString("wrongSignIn"),
							resourceBundle.getString("badCredentials"),
							JOptionPane.INFORMATION_MESSAGE);
					logger.info(resourceBundle.getString("wrongUsernamePassword") + ": " + textFieldEmail.getText().trim() + " " + textFieldPassword.getText().trim());
					
				}
				
				//Comprobamos si el usuario es el ADMIN
				if(textFieldEmail.getText().trim().equals("admin") && textFieldPassword.getText().trim().equals("admin")){
